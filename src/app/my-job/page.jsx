...
const MyJobs = () => {
  ...
  return (
    <div className="max-w-screen-2xl container mx-auto xl:px-24 px-4 min-h-[80vh] mt-8">
          ...
          <div className="relative flex flex-col min-w-0 break-words bg-white w-full mb-6 shadow-lg rounded ">
            <div className="rounded-t mb-0 px-4 py-3 border-0 bg-white shadow">
              <div className="flex flex-row justify-between items-center space-y-4">
                ...
                <div className="w-full sm:w-auto flex flex-col sm:flex-row items-center justify-end space-y-2 sm:space-y-0 sm:space-x-2">
                  ...
                  <Link href="/post-job">
                    <button className="bg-sky-500 text-white active:bg-sky-600 text-sm font-bold uppercase p-3 rounded-md outline-none focus:outline-none w-auto ease-linear transition-all duration-150">
                      Post A New Job
                    </button>
                  </Link>
                </div>
            ...
            <div className="block w-full overflow-x-auto">
              <table className="items-center bg-transparent w-full border-collapse">
                <thead>
                  <tr>
                    {['No', 'Title', 'Company Name', 'Salary', 'Edit', 'Delete'].map((header) => (
                      <th key={header} className="px-6 bg-blueGray-50 text-blueGray-500 align-middle border border-solid border-blueGray-100 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">
                        {header}
                      </th>
                    ))}
                  </tr>
                      ...
                      <tr key={job?._id || index}>
                        ...
                        <td className="border-t-0 px-6 text-xs whitespace-nowrap p-4">
                          {job?.minPrice && job?.maxPrice ? (
                            <>
                              <i className="fas fa-arrow-up text-emerald-500 mr-2"></i>£ {job.minPrice} - £ {job.maxPrice}
                            </>
                          ) : (
                            "No Salary Info"
                          )}
                        ...
                        <td className="border-t-0 px-6 text-xs whitespace-nowrap p-4">
                          <Link href={`/edit-job/${job?._id}`}>
                            <button className="bg-sky-500 py-2 px-6 text-white rounded-sm">
                              Edit
                            </button>
                          </Link>
                        </td>
