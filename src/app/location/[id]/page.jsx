...
const Location = () => {
    ...
    const fetchJobs = async () => {
      ...
      try {
        const response = await fetch(
          `${BASE_URL}/location-jobs/${id}?page=${currentPage + 1}&limit=${jobsPerPage}`
        );
  ...
  const JobCard = ({ job }) => {
    ...
    return (
      <section className='card border border-gray-300 rounded mb-4 hover:shadow-lg p-3'>
  ...
  return (
    <>
      <Head>
        <title>
          Jobs in {id || "Job"} - Full-Time, Part-Time, Remote & Freelance Roles | Aidifys.com
        </title>
        <meta
          name="description"
          content={`Browse the latest job vacancies in ${id || "various industries"},  Find full-time, 
      ...
      >
        <div className="flex justify-center mt-8">
          <div className="w-[80vw] min-h-[80vh]">
            <div>
              <h3 className="text-lg font-bold mb-2 ml-6">{jobCount} Jobs in {id}</h3>
            </div>
            {jobs.map(job => (
